rule Trojan_AndroidOS_Koler_TA_MSR{
	meta:
		description = "Trojan:AndroidOS/Koler.TA!MSR,SIGNATURE_TYPE_DEXHSTR_EXT,0a 00 0a 00 02 00 00 "
		
	strings :
		$a_02_0 = {22 00 33 00 1c 01 ?? 03 70 30 da 00 40 01 6e 20 d6 00 04 00 5b 34 ?? 06 22 01 33 00 1c 02 ?? 03 70 30 da 00 41 02 5b 31 ?? 06 1c 01 ?? 03 70 20 ?? ?? 13 00 0a 01 39 01 07 00 54 31 87 06 6e 20 d6 00 14 00 6e 10 e0 00 05 00 0c 01 1a 02 06 0d 6e 20 ?? ?? 21 00 0a 01 38 01 07 00 54 31 ?? 06 6e 20 ?? ?? 41 00 0e 00 } 		$a_02_1 = {12 10 0f 00 90 0a 40 00 69 02 ?? 06 54 20 ?? 06 54 21 ?? 06 6e 30 ?? ?? 02 01 22 00 ?? ?? 70 20 ?? ?? 20 00 5b 20 ?? 06 12 10 0f 00 } 	condition:
		((#a_02_0  & 1)*5+(#a_02_1  & 1)*5) >=10
 
}
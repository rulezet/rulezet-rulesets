rule TrojanDownloader_O97M_Obfuse_EI{
	meta:
		description = "TrojanDownloader:O97M/Obfuse.EI,SIGNATURE_TYPE_MACROHSTR_EXT,0b 00 0b 00 02 00 00 "
		
	strings :
		$a_03_0 = {47 65 74 4f 62 6a 65 63 74 28 [0-50] 29 2e 43 72 65 61 74 65 20 [0-10] 2e [0-25] 20 2b 20 } 		$a_03_1 = {53 68 6f 77 57 69 6e 64 6f 77 20 3d 20 [0-14] 20 } 	condition:
		((#a_03_0  & 1)*10+(#a_03_1  & 1)*1) >=11
 
}
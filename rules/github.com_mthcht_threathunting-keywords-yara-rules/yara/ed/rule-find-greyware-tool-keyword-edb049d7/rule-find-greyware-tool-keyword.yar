rule rule_find_greyware_tool_keyword
{
    ...
    strings:
                        $string1_find_greyware_tool_keyword = /\s\/var\/log\s\-type\sf\s\-exec\s.{0,100}\/tr.{0,100}\s\-s\s0\s\{\}\s\\/
                        ...
                        $string11_find_greyware_tool_keyword = /find\s\/\s\-name\sauthorized_keys\s.{0,100}\>\s\/dev\/null/
                        $string12_find_greyware_tool_keyword = "find / -name ftp"
                        $string13_find_greyware_tool_keyword = "find / -name id_dsa 2>"
                        $string14_find_greyware_tool_keyword = /find\s\/\s\-name\sid_rsa\s.{0,100}\>\s\/dev\/null/
                        $string15_find_greyware_tool_keyword = "find / -name id_rsa 2>"
                        $string16_find_greyware_tool_keyword = "find / -name netcat"
                        $string17_find_greyware_tool_keyword = /find\s\/\s\-name\stftp.{0,100}\s/
                        ...
                        $string32_find_greyware_tool_keyword = /find\s\/\s\-perm\s\-u\=s\s\-type\sf\s\-group\s.{0,100}\/dev\/null/
                        ...
                        $string42_find_greyware_tool_keyword = /find\s\/.{0,100}\s\-perm\s\-04000\s\-o\s\-perm\s\-02000/
                        $string43_find_greyware_tool_keyword = /find\s\/.{0,100}\s\-perm\s\-u\=s\s\-type\sf\s2\>/

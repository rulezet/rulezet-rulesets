rule _Virus_Hijack_Gen_Trojan_ShellObject_dyX_ai9jxwp_1_1_Virus_Hijac_421 {
  meta:
  ...
  strings:
    $s1 = "Failed to query service configuration" fullword wide
    $s2 = "Failed to read %s registry value with error %d" fullword wide
    $s3 = "Failed to query %s registry value, incorrect type %d" fullword wide
    $s4 = "Failed to open Sysmon parameters key with error %d" fullword wide
    $s5 = "gUnknown Event type" fullword wide
    $s6 = "Unkown Event Type" fullword wide

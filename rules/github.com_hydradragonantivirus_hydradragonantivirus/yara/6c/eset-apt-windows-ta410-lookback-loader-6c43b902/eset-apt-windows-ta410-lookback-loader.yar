rule ESET_Apt_Windows_TA410_Lookback_Loader: FILE {
  meta:
    description = "Matches the modified function in LookBack libcurl loader."
    author      = "ESET Research"
    id          = "d0aac4f6-f72f-5adf-8f8f-9251bad70131"
    date        = "2021-10-12"
    modified    = "2022-04-27"
    reference   = "https://github.com/eset/malware-ioc/"
    source_url  = "https://github.com/eset/malware-ioc/blob/21381c70ad030105cf9edb092dfd1cae29753286/ta410/ta410.yar#L256-L309"
    license_url = "https://github.com/eset/malware-ioc/blob/21381c70ad030105cf9edb092dfd1cae29753286/LICENSE"
    logic_hash  = "98390dd664227ad747e5572771d12e7ebd2475d26db27e85508347ac6f44f3bf"
    score       = 75
    quality     = 80
    tags        = "FILE"
    license     = "BSD 2-Clause"
    version     = "1"

  strings:
    $chunk_1 = {
      FF 15 ?? ?? ?? ??        6A 40                    68 00 10 00 00           68 F0 04 00 00           6A 00                    FF 15 ?? ?? ?? ??        8B E8                    B9 3C 01 00 00           BE 60 30 06 10           8B FD                    68 F0 04 00 00           F3 A5                    55                       E8 ?? ?? ?? ??           8B 0D ?? ?? ?? ??        A1 ?? ?? ?? ??           68 6C 02 00 00           89 4C 24 ??              89 44 24 ??              FF 15 ?? ?? ?? ??        8B D8                    B9 9B 00 00 00           BE 50 35 06 10           8B FB                    68 6C 02 00 00           F3 A5                    53                       E8 ?? ?? ?? ??           83 C4 14                 8D 44 24 ??              50                       53                       8D 44 24 ??              50                       A1 ?? ?? ?? ??           FF 74 24 ??              03 C5                    FF D0                  }

  condition:
    uint16(0) == 0x5a4d and all of them
}
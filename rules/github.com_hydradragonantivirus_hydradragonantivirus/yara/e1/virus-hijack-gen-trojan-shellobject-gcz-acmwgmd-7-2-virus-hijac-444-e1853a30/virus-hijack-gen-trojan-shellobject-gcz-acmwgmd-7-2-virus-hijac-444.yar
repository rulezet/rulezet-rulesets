rule _Virus_Hijack_Gen_Trojan_ShellObject_gCZ_aCmWGmd_7_2_Virus_Hijac_444 {
  meta:
  ...
  strings:
    $s1  = "Result of ExecuteShell: HR = 0x%08x" fullword wide
    $s2  = "Result of ExecuteLegacyRemediations: HR = 0x%08x" fullword wide
    $s3  = "Result of ExecuteEventDrivenRemediations: HR = 0x%08x" fullword wide
    $s4  = "The sediment pack service failed to pause" fullword wide
    $s5  = "The sediment pack service failed to continue" fullword wide
    $s6  = "The sediment pack service failed to shutdown" fullword wide
    $s7  = "Sediment service started" fullword wide
    $s8  = "Sediment service stopped" fullword wide
    $s9  = "Current memory bytes %lli" fullword wide
    $s10 = "Result of initializing onesettings: HR = 0x%08x" fullword wide

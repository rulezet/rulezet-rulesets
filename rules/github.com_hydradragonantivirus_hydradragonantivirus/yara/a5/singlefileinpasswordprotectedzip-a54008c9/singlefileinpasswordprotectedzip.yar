rule SingleFileInPasswordProtectedZip {
  ...
  condition:
    uint32be(0) == 0x504b0304      and for any i in (0..0x100): (        uint32be(filesize - i) == 0x504b0506        and uint16(filesize - i + 0xa) == 1        and uint32be(uint32(filesize - i + 0x10)) == 0x504b0102        and uint16(uint32(filesize - i + 0x10) + 8) & 1 == 1      )
}